...
class Settings(BaseSettings):
    ...
    # Media Processing & Concurrency Configuration (Phase 21)
    MAX_MEDIA_RETRIES: int = Field(default=3, description="Maximum retry attempts for failed media processing")
    MEDIA_WORKER_CONCURRENCY: int = Field(default=2, description="Number of concurrent media worker tasks")
    AI_WORKER_CONCURRENCY: int = Field(default=3, description="Number of concurrent AI tasks")
    PUBLISH_WORKER_CONCURRENCY: int = Field(default=5, description="Number of concurrent publishing tasks")
    WEBHOOK_WORKER_CONCURRENCY: int = Field(default=5, description="Number of concurrent webhook delivery tasks")
    MAX_QUEUE_DEPTH: int = Field(default=100, description="Maximum pending jobs before backpressure 429/503 rejection")
    MAX_STORAGE_GB: float = Field(default=50.0, description="Maximum total storage quota in GB")
    TEMP_STORAGE_LIMIT_GB: float = Field(default=10.0, description="Maximum temporary file storage limit in GB")
    JOB_TIMEOUT_SECONDS: int = Field(default=600, description="Max execution timeout in seconds for queue jobs")
    FFMPEG_TIMEOUT_SECONDS: int = Field(default=300, description="Max execution timeout in seconds for FFmpeg process")
    AI_TIMEOUT_SECONDS: int = Field(default=60, description="Max execution timeout in seconds for AI requests")
    DB_POOL_SIZE: int = Field(default=20, description="Database async connection pool size")
    DB_MAX_OVERFLOW: int = Field(default=10, description="Database async connection pool max overflow")
    DB_POOL_TIMEOUT: int = Field(default=30, description="Database connection pool checkout timeout")
    DB_POOL_RECYCLE: int = Field(default=1800, description="Database connection pool recycle time in seconds")
    MAX_BULK_OPERATION_SIZE: int = Field(default=50, description="Maximum bulk operation batch size")
